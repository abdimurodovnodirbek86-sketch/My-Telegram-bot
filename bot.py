...
async def post_movie_to_channel(title, description, category, code, file_id, poster_file_id, share_link, is_vip=False, is_premium=False, episode_number=None):
    """Yangi kino/serial qismi qo'shilishi bilan TO'LIQ VIDEONI kino kanaliga chiroyli dizaynda joylaydi"""
    if not POST_CHANNEL:
        return
    try:
        ...
        if poster_file_id:
            # Avval poster (afisha) alohida chiroyli e'lon sifatida, keyin to'liq video
            await bot.send_photo(POST_CHANNEL, poster_file_id, caption=caption, parse_mode="HTML", reply_markup=kb.as_markup())
            await bot.send_video(POST_CHANNEL, file_id, caption=f"▶️ {title}{ep_line}", parse_mode="HTML")
        else:
            await bot.send_video(POST_CHANNEL, file_id, caption=caption, parse_mode="HTML", reply_markup=kb.as_markup())
    except Exception as e:
        logging.exception(f"Kino kanaliga video post xatosi: {e}")
...
def save_post_channel_db(channel):
    db_execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
               ("post_channel", channel), commit=True)
