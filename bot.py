...
def init_userbot_table():
    """Faqat admin uchun: shaxsiy Telegram sessiyalari saqlanadigan jadval"""
...
def _ub_menu_keyboard(is_connected: bool):
    ...
    if is_connected:
        ...
        builder.button(text="🔐 Ulanish (login)", callback_data="userbot_login_start")
...
async def userbot_receive_phone(message: Message, state: FSMContext):
    ...
    if not USERBOT_PHONE_REGEX.match(phone):
        await message.answer(
            "❌ Format noto'g'ri. Namuna: <code>+998901234567</code>. Qaytadan yuboring:",
            parse_mode="HTML",
        )
        return
    try:
        ...
        await message.answer("❌ Bu telefon raqami Telegram'da mavjud emas. Qaytadan urinib ko'ring:")
    except FloodWaitError as e:
        await message.answer(f"⏳ Juda ko'p urinish. {e.seconds} soniyadan keyin qaytadan urinib ko'ring.")
        await state.clear()
    except Exception as e:
...
async def userbot_receive_code(message: Message, state: FSMContext):
    ...
    if not pending:
        await message.answer("❌ Login sessiyasi topilmadi. /admin_userbot orqali qaytadan boshlang.")
        await state.clear()
        return
    ...
    try:
        ...
        await _ub_finalize_login(message, state, admin_id, pending["phone"], client)
        ...
        await message.answer("❌ Kod noto'g'ri yoki muddati o'tgan. Qaytadan kiriting:")
    except Exception as e:
...
async def _ub_finalize_login(message: Message, state: FSMContext, admin_id: int, phone: str, client):
    ...
    await message.answer(
        f"✅ Muvaffaqiyatli ulandi: <b>{name}</b> ({phone})",
        parse_mode="HTML",
