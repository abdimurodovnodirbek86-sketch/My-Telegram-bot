        ...
        if user.id not in ADMIN_IDS:
            ...
            if reg_status == "pending":
                ...
                if full_name and reg_photo:
                    ...
                    if WEBAPP_URL:
                        kb = InlineKeyboardBuilder()
                        kb.button(text="📸 Kamerani ochish", web_app=WebAppInfo(url=f"{WEBAPP_URL}/register"))
                        kb.adjust(1)
                        await message.answer(
                            "📸 Ro'yxatdan o'tishni yakunlash uchun tugmani bosing:",
                            reply_markup=kb.as_markup()
                        )
                    else:
                        await message.answer(
...
async def registration_name_received(message: Message, state: FSMContext):
    try:
        ...
        if WEBAPP_URL:
            ...
            kb.adjust(1)
            await message.answer(
                "📸 Rahmat! Endi tasdiqlash uchun quyidagi tugmani bosing — kamera avtomatik ochiladi:",
                reply_markup=kb.as_markup()
            )
        else:
            await message.answer(
...
<title>Yuz tekshiruvi</title>
<script src="https://telegram.org/js/telegram-web-app.js"></script>
<style>
  * { box-sizing: border-box; }
  body {
    margin:0; padding:0; min-height:100vh;
    background: radial-gradient(circle at 50% 20%, #0f2027, #0a0f14 70%);
    color:#fff; font-family: -apple-system, Segoe UI, Roboto, sans-serif;
    text-align:center; display:flex; flex-direction:column; align-items:center; padding:20px 16px;
  }
  h2 { font-weight:600; margin:6px 0 18px; letter-spacing:0.3px; }
  #stage {
    position:relative; width:100%; max-width:340px; aspect-ratio: 3/4;
    border-radius:20px; overflow:hidden; background:#000;
    box-shadow: 0 0 0 1px rgba(46,166,255,0.25), 0 20px 40px rgba(0,0,0,0.5);
  }
  ...
  #scanline {
    ...
    background: linear-gradient(90deg, transparent, #3ee1ff, #7dffb3, #3ee1ff, transparent);
    box-shadow: 0 0 12px 3px rgba(62,225,255,0.85);
  ...
  /* Burchak markerlar (viewfinder) */
  .corner { position:absolute; width:34px; height:34px; border:3px solid #3ee1ff; opacity:0.9; filter: drop-shadow(0 0 6px rgba(62,225,255,0.8)); }
  ...
  /* Yuz oval qo'llanmasi */
  #ovalGuide {
    position:absolute; left:50%; top:46%; transform:translate(-50%,-50%);
    width:62%; height:56%; border:2px dashed rgba(255,255,255,0.35); border-radius:50%;
  }

  #recDot {
    position:absolute; top:14px; left:50%; transform:translateX(-50%);
    display:flex; align-items:center; gap:6px; background:rgba(0,0,0,0.5);
    padding:5px 12px; border-radius:20px; font-size:12px; letter-spacing:0.5px;
  }
  #recDot span.dot { width:8px; height:8px; border-radius:50%; background:#ff4757; animation:blink 1s infinite; }
  @keyframes blink { 0%,100%{opacity:1;} 50%{opacity:0.2;} }

  #progressWrap { width:100%; max-width:340px; margin-top:18px; background:rgba(255,255,255,0.08); border-radius:20px; height:10px; overflow:hidden; }
  #progressBar { height:100%; width:0%; background: linear-gradient(90deg,#3ee1ff,#7dffb3); transition: width 0.15s linear; }

  #status { margin-top:16px; font-size:15px; min-height:22px; color:#cdefff; letter-spacing:0.2px; }
  #percent { font-size:13px; color:#7dffb3; margin-top:4px; }

  #successMark {
    display:none; font-size:54px; margin-top:6px; color:#22c55e;
...
const RECORD_MS = 4000; // skanerlash/yozish davomiyligi
let stream = null;
...
async function startCamera() {
  try {
    ...
    statusEl.textContent = "❌ Kameraga ruxsat berilmadi: " + e.message;
  }
}
...
async function onRecordingStop() {
  ...
  reader.onloadend = async () => {
    await uploadVideo(reader.result);
  };
...
async function uploadVideo(base64data) {
  ...
  try {
    const resp = await fetch('/submit-scan', {
      ...
      body: JSON.stringify({ initData: tg.initData, video: base64data })
    });
    const result = await resp.json();
    if (result.ok) {
      progressBar.style.width = '100%';
      percentEl.textContent = '100%';
      statusEl.textContent = "✅ Yuborildi! Admin tez orada ko'rib chiqadi.";
...
async def register_page(request):
    """Kamera orqali ro'yxatdan o'tish Mini App sahifasi (skanerlash effektlari bilan)"""
    return web.Response(text=REGISTER_PAGE_HTML, content_type="text/html")

async def submit_scan(request):
    """Mini App'dan kelgan skanerlash videosini tekshirib, adminlarga yuboradi"""
    try:
        data = await request.json()
        init_data = data.get("initData", "")
        video_data_url = data.get("video", "")

        user_info = validate_webapp_init_data(init_data)
        if not user_info:
            return web.json_response({"ok": False, "error": "Imzo tekshiruvi muvaffaqiyatsiz"}, status=403)
        ...
        log_action(user_id, "registration_submitted", "webapp video scan")
