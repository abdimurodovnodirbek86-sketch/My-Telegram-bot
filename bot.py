...
<style>
  * { box-sizing: border-box; }
  ...
  .opp { background:#123528aa; border:1px solid #D4A72C33; border-radius:10px; padding:8px 10px; min-width:88px; flex-shrink:0; font-size:12px; }
  .opps { display:flex; gap:8px; overflow-x:auto; margin-bottom:10px; }
...
function renderGame(){
  ...
  const opps = document.createElement('div'); opps.className='opps';
  r.players.filter(p => p.id !== STATE.playerId && !p.isSpectator).forEach(p => {
    ...
    box.innerHTML = `<b>${p.id===r.hostId?'👑 ':''}${p.name}</b>${r.cheers[p.id]?` 👏${r.cheers[p.id]}`:''}<br><span class="tiny">${p.id===r.defenderId?'himoyachi':p.id===r.roundAttackerId?'hujumchi':''}</span><br><span class="tiny">${cnt} karta</span>`;
    opps.appendChild(box);
  });
  wrap.appendChild(opps);

  if (r.status === 'playing') {
    const tr = document.createElement('div'); tr.style.cssText='display:flex;gap:10px;justify-content:center;margin-bottom:10px;';
    const tc = document.createElement('div'); tc.className='center';
    tc.appendChild(cardEl(r.trumpCard)); tc.innerHTML += `<div class="tiny">ko'zir</div>`;
    const dc = document.createElement('div'); dc.className='center';
    dc.appendChild(backCardEl()); dc.innerHTML += `<div class="tiny">${r.deck.length} karta</div>`;
    tr.appendChild(tc); tr.appendChild(dc);
    wrap.appendChild(tr);
  }
  ...
  if (r.status === 'playing' && !me.isSpectator) {
    ...
    if (isDefender && r.table.some(t=>!t.defend)) {
      const b = document.createElement('button'); b.className='btn-danger'; b.textContent='🖐 Olaman';
  ...
  if (!me.isSpectator) {
    ...
    (me.hand||[]).forEach(c => {
      hand.appendChild(cardEl(c, {sel: STATE.selCard===c.id, onClick: () => { STATE.selCard = STATE.selCard===c.id ? null : c.id; render(); }}));
    });
