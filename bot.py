...
def log_invite_claim(group_id, claimer_id, target_type, target_value, resolved_user_id, count):
    db_execute(
        "INSERT INTO invite_claims_log (group_id, claimer_id, target_type, target_value, resolved_user_id, count) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (group_id, claimer_id, target_type, target_value, resolved_user_id, count), commit=True,
    )
...
async def ie_cmd_tasks(message: Message):
    ...
    for task_id, owner_id, channel_link, required_subs, current_subs, status, created_at in tasks:
        ...
        await message.answer(
            ...
            f"🎁 Mukofot: {INVITE_COIN_REWARD} ball",
            reply_markup=kb.as_markup(),
        )
...
async def ie_complete_task_callback(call: CallbackQuery):
    ...
    complete_invite_task(task_id, user_id, INVITE_COIN_REWARD)
    await call.answer(f"✅ Rahmat! Balansingizga {INVITE_COIN_REWARD} ball qo'shildi.", show_alert=True)
...
async def ie_on_new_chat_members(message: Message):
    ...
    credit_invite(inviter.id, len(new_members))
...
async def ie_on_left_chat_member(message: Message):
    ...
    if left_user.is_bot:
        return
    result = penalize_departure(message.chat.id, left_user.id, LEFT_MEMBER_PENALTY)
    if not result:
        return
    ...
    await message.answer(
        ...
        f"👤 Uni qo'shgan: {who}\n"
        f"💸 Jarima: -{LEFT_MEMBER_PENALTY} ball (yangi balans: {new_balance}){warning}"
    )
...
async def ie_on_group_text(message: Message):
    ...
    if now - last < CLAIM_COOLDOWN_SECONDS:
        await message.reply(f"⏳ Juda tez-tez da'vo yubordingiz. {int(CLAIM_COOLDOWN_SECONDS - (now - last))} soniyadan keyin urinib ko'ring.")
        return
    ...
    if target_type == "id":
        ...
        matches = find_users_by_name_ie(target_value)
        if len(matches) == 0:
            await message.reply(
                f"🤖 <b>AI Tizim:</b> \"{target_value}\" ismli foydalanuvchi bazada topilmadi. "
            ...
            return
        if len(matches) > 1:
            names = ", ".join(f"{m[2]} (@{m[1]})" if m[1] else m[2] for m in matches)
            await message.reply(
            ...
            return
        m = matches[0]
        resolved_user = (m[0], m[2], m[1])  # user_id, first_name, username
    ...
    credit_invite(resolved_id, count)
    ...
    await message.reply(
        f"🤖 <b>AI Tizim:</b> Ma'no tushunildi! <b>{count} ball</b> — <b>{who}</b> foydalanuvchiga yozildi ✅",
        parse_mode="HTML",
    )
