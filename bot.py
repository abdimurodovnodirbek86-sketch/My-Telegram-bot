...
<style>
  ...
  #video, #canvas { width:100%; max-width:480px; border-radius:12px; margin-top:12px; }
  button { padding:14px 24px; font-size:16px; border:none; border-radius:10px; margin:10px 6px; cursor:pointer; }
  #shoot { background:#2ea6ff; color:#fff; }
  #retake { background:#555; color:#fff; }
  #send { background:#22c55e; color:#fff; }
  #status { margin-top:16px; font-size:14px; color:#aaa; }
  #canvas { display:none; }
</style>
</head>
<body>
  <h3>📸 Ro'yxatdan o'tish uchun selfie oling</h3>
  <video id="video" autoplay playsinline muted></video>
  <canvas id="canvas"></canvas>
  <div>
    <button id="shoot" disabled>⏳ Kamera yuklanmoqda...</button>
    <button id="retake" style="display:none">🔄 Qayta olish</button>
    <button id="send" style="display:none">✅ Yuborish</button>
  </div>
  <div id="status"></div>
...
async function startCamera() {
  try {
    ...
    video.onloadedmetadata = async () => {
      try { await video.play(); } catch (e) { /* ba'zi brauzerlarda avtomatik ijro cheklangan bo'lishi mumkin */ }
      // videoWidth/Height tayyor bo'lguncha kutamiz (ba'zi qurilmalarda kechikish bo'ladi)
      const waitReady = setInterval(() => {
        if (video.videoWidth > 0 && video.videoHeight > 0) {
          clearInterval(waitReady);
          cameraReady = true;
          shootBtn.disabled = false;
          shootBtn.textContent = "📷 Suratga olish";
        }
      }, 150);
    };
  } catch (e) {
...
shootBtn.onclick = () => {
  if (!cameraReady || video.videoWidth === 0) {
    statusEl.textContent = "⏳ Kamera hali tayyor emas, biroz kuting...";
    return;
  }
...
sendBtn.onclick = async () => {
  ...
  try {
    const resp = await fetch('/submit-selfie', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ initData: tg.initData, photo: photoDataUrl })
    });
    const result = await resp.json();
    if (result.ok) {
      statusEl.textContent = "✅ Yuborildi! Botga qaytishingiz mumkin.";
...
async def register_page(request):
    """Kamera orqali ro'yxatdan o'tish Mini App sahifasi"""
    return web.Response(text=REGISTER_PAGE_HTML, content_type="text/html")

async def submit_selfie(request):
    """Mini App'dan kelgan suratni tekshirib, adminlarga yuboradi"""
    try:
        ...
        if not photo_data_url or "," not in photo_data_url:
            return web.json_response({"ok": False, "error": "Surat topilmadi"}, status=400)

        photo_bytes = base64.b64decode(photo_data_url.split(",", 1)[1])
        db_execute("UPDATE users SET registration_photo=? WHERE user_id=?", ("webapp_camera", user_id), commit=True)
        log_action(user_id, "registration_submitted", "webapp camera selfie")
        ...
        kb.adjust(2)
        photo_file = BufferedInputFile(photo_bytes, filename="selfie.jpg")
        for admin_id in ADMIN_IDS:
            try:
                await bot.send_photo(
                    admin_id, photo_file,
                    caption=(
                        f"🆕 <b>Yangi ro'yxatdan o'tish so'rovi (kamera orqali)</b>\n\n"
                        f"👤 F.I.Sh: {full_name}\n"
                        f"🆔 ID: <code>{user_id}</code>\n"
                        f"📱 Username: @{username_display}"
                    ),
        ...
        logging.exception(f"Selfie qabul qilish xatosi: {e}")
...
async def start_web_server():
    ...
    app.router.add_post("/submit-selfie", submit_selfie)
